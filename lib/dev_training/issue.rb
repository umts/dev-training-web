...
module DevTraining
  ...
  class Issue
    ...
    def find
      repo_name = @repo.resource.full_name
      milestone_number = @milestone.resource.number
      @client.issues(repo_name, milestone: milestone_number).find { |issue| issue.title == @title }
    end
  end
end
